...
Below we add two modules ([`clicks.R` and `message.R`](https://appsilon.github.io/rhino/articles/tutorial/write-end-to-end-tests-with-cypress.html#test-application)) to the `rap` application (i.e., in `app/view`). Both modules contain an `actionButton()` and `textOutput()`. Using `app/view/clicks` or `app/view/message` requires adding both modules in the `app/main.R` file with `box::use()`.
