...
`rhino` apps are unique because unlike `golem` and `leprechaun`, they aren't R packages. `rhino` uses [`box`](https://klmr.me/box/) modules to manage add-on package functions, which makes simplifies app dependencies and separates the code into a clear 'division of labor.'[^view-rap]
...
#| eval: false 
#| code-fold: true 
#| code-summary: 'view monolithlic app.R file'
ui <- shiny::fluidPage(
...
#| eval: false
#| code-fold: true 
#| code-summary: 'view scatter_plot.R utility function'
...
The methods above beg the questions:

1. _Why do we load the **entire** package namespace *and* attaching it to the search list with `library`, even if we only need a single function during standard (i.e., non-package) R development?_ [^install-packages-library]
...
[^install-packages-library]: `install-packages()` downloads and installs packages from a repo like CRAN or GitHub, and `library` (or `require`) loads and attaches the add-on packages.

[^import-packages-description]: Adding a package to the `Imports` field in the `DESCRIPTION` will download/install the add-on package when your package is installed, but not *attach* it to the search list. 
...
Below is a quick demonstration of how `box` modules work using the `tidyverse_logo()` function from the `tidyverse` meta-package. If we attempted to use the `tidyverse_logo()` function without installing or loading the `tidyverse` package, we see the following error:
...
`box` modules are self-contained, meaning the `tidyverse_logo()` function only exists *inside* the `logo` module. 
...
Explicitly listing the packages and functions we intend to use with `box::use()` means we no longer need to include calls to  `install.packages()` and `library()`/`require()`. This was a very brief overview of `box`, so I *highly recommend* consulting the [`box` website](https://klmr.me/box/index.html) and vignettes.[^box-read-more]
...
[^box-read-more]: I highly recommend the ["the hierarchy of module environments"](https://klmr.me/box/articles/mod-env-hierarchy.html) vignette. The `rhino` website also has a great overview on using [`box` with shiny apps](https://appsilon.github.io/rhino/articles/explanation/box-modules.html), and I've also created a collection of `box` module examples in this [repo](https://github.com/mjfrigaard/rbox/blob/main/boxes.md))
...
Testing `rhino` apps is similar to any `testthat` folder (and can be used with [`shinytest2`](https://appsilon.github.io/rhino/articles/how-to/use-shinytest2.html)) and comes with a helper function, [`rhino::test_r()`](https://appsilon.github.io/rhino/reference/test_r.html)
