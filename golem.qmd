...
-   `golem::use_recommended_tests()` adds the `spelling` package to our `DESCRIPTION` and updates the `WORDLIST`[^golem-tests]
...
`mod_var_input` collects the reactive inputs from the UI and passes them to `mod_scatter_display` (you can view the code in these modules [here on GitHub](https://github.com/mjfrigaard/sfw/tree/main/_apps/gap)).

### Utility functions

The `scatter_plot()` utility function file was created with `golem::add_utils()` (and ends with a `_utils` suffix):
...
The `covrpage` package provides a test coverage report in `tests/README.md` file.[^test-coverage]
