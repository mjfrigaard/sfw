...
Utility functions can also be created directly with [`golem::add_utils()` or `golem::add_fct()`](https://thinkr-open.github.io/golem/reference/file_creation.html).[^golem-remove-nord]
...
::: {#fig-golem_app_initial .column}
![Initial `gap::run_app()`](img/golem_app_initial.png){#fig-golem_app_initial width="100%" fig-align="center"}

The initial application in the `gap` package
:::
...
co_box(color = "g", 
  fold = TRUE, 
  header = "Using the `inst/` folder", 
  contents = "`golem` takes advantage of the `inst/` folder and R package structure to allow users to provide additional 'assets' to the application. 
...
`golem` has multiple functions for [creating](https://thinkr-open.github.io/golem/reference/index.html#create-files) and [using](https://thinkr-open.github.io/golem/reference/index.html#use-files) external files in your Shiny app-package. The `add_dockerfile*` set of functions are particularly helpful for including Docker files. I'll cover two here: `golem::add_dockerfile()` and `golem::add_dockerfile_with_renv()`. 

In the `gap` directory, you'll see the [`Dockerfile`](https://github.com/mjfrigaard/sfw/blob/main/_apps/gap/Dockerfile) from `add_dockerfile()`. The files from `add_dockerfile_with_renv()` are in the [`inst/docker-renv` folder.](https://github.com/mjfrigaard/sfw/tree/main/_apps/gap/inst/docker-renv).

#### [`add_dockerfile`]{style="font-size: 0.82em; font-weight: bold;"}
...
#### [`add_dockerfile_with_renv`]{style="font-size: 0.82em; font-weight: bold;"}
...
The `docker build` and `docker run` commands in the [`README`]{style="font-size: 1.01em; font-weight: bold; font-style: italic;"} create the base image from `Dockerfile_base`, start the container from the `Dockerfile` image, and launche the `gap` application on port `80`.
