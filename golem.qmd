...
The [`golem` package](https://thinkr-open.github.io/golem/) is an 'opinionated Shiny framework' that provides many options for users experienced with creating R packages, but who are looking to streamline their application development.[^view-gap]
...
The [`dev/`](https://github.com/mjfrigaard/sfw/tree/main/_apps/gap/dev) folder contains `golem`'s 'guided tour' scripts and `dev/run_dev.R`:

Below are the scripts to guide you through developing your `golem` app and the `dev/run_dev.R` code.

- [x] `dev/01_start.R` opens automatically
...
-  [x] `fill_desc()` uses the [`desc` package](https://r-lib.github.io/desc/) and the sections are entered in a `key = "value"` format
...
- [x] LICENSE: `usethis::use_mit_license()`  

- [x] README: `usethis::use_readme_rmd()`   

- [x] Code of Conduct: `usethis::use_code_of_conduct()`

- [x] Lifecycle badge: `usethis::use_lifecycle_badge("Experimental")`

- [x] `NEWS.md`: `usethis::use_news_md(open = FALSE)`

- [x] Git: `usethis::use_git()`

### `golem` files

The `golem` functions in `dev/01_start.R` are for setting the `golem` options and using recommended tests. 

- [x] Options: `golem::set_golem_options()`
    
- [x] Tests (with `testthat`): `golem::use_recommended_tests()`
    
- [x] Favicon: `golem::use_favicon()`
    
- [x] Helper functions:   [`golem::use_utils_ui()`](https://thinkr-open.github.io/golem/reference/utils_files.html) and [`golem::use_utils_server()`](https://thinkr-open.github.io/golem/reference/utils_files.html) create `golem`'s UI (`R/golem_utils_ui.R`) and server (`R/golem_utils_server.R`) utility functions in the `R/` folder
...
- [x] `R/app_ui.R` and `R/app_server.R` are `golem`'s version of `ui.R` and `server.R`  
...
- [x] `golem_add_external_resources()` is a wrapper for `shiny::addResourcePath()`, `htmltools::htmlDependency()`, and `app_sys()` (which is a wrapper for `system.file()`):
...
- [x] `run_app.R` is an exported function that is available for me to run my app after I've installed the package: 
...
- [x] `golem` has wrappers for creating modules and helper functions in the `R/` folder: 
...
- [x] The [`R/app_config.R` file](https://github.com/mjfrigaard/moviesApp/blob/main/gap/R/app_config.R) contains two functions: `app_sys()` (covered above) and `get_golem_config()`, which reads the `inst/golem-config.yml` configuration file
...
- [x] [`golem-config.yml`](https://engineering-shiny.org/golem.html#manipulating-golem-config.yml) gives access to the app version, name, and (development) working directory, so it can be used to add "production-only elements" and is "shareable across golem projects"\
    
- [x] `golem` uses `app_sys()` (a wrapper around `system.file()`) to add external resources to the application (`app_sys()` is included in the `R/app_config.R` file)
...
- [x] `get_golem_config()` is also included in the `R/app_config.R` file
...
- [x] `golem::use_recommended_tests()` adds the `spelling` package to our `DESCRIPTION` and updates the `WORDLIST`[^golem-tests]

[^golem-tests]: The tests `golem` creates in the `tests/testthat/` folder can serve as a nice guide for users new to `testthat`

- [x] The `tests` folder uses the [`testthat` framework](https://testthat.r-lib.org/)
...
- [x] [`golem::add_js_file("script")`](https://thinkr-open.github.io/golem/reference/add_files.html)

- [x] [`golem::add_js_handler("handlers")`](https://thinkr-open.github.io/golem/reference/add_files.html)

- [x] [`golem::add_css_file("custom")`](https://thinkr-open.github.io/golem/reference/add_files.html)

- [x] [`golem::add_sass_file("custom")`](https://thinkr-open.github.io/golem/reference/add_files.html)
...
-   [x] [`golem::add_rstudioconnect_file()`](https://thinkr-open.github.io/golem/reference/rstudio_deploy.html), [`golem::add_shinyappsio_file()`](https://thinkr-open.github.io/golem/reference/rstudio_deploy.html), and [`golem::add_shinyserver_file()`](https://thinkr-open.github.io/golem/reference/rstudio_deploy.html)

Docker
<br>

-   [x] [`golem::add_dockerfile_with_renv()`](https://thinkr-open.github.io/golem/reference/dockerfiles.html) and [`golem::add_dockerfile_with_renv_shinyproxy()`](https://thinkr-open.github.io/golem/reference/dockerfiles.html)
...
- [x] `R/mod_name.R`: a boilerplate Shiny module (ui and server functions)
...
- [x] `R/mod_name_fct_fun.R` and `R/mod_name_utils_fun.R`: empty R files in the R/ folder (with the same prefix as the module).

- [x] `tests/testthat/test-mod_name.R`: a test file for the module (with some boilerplate tests)
...
View the modules I use in the `gap` application [here on GitHub](https://github.com/mjfrigaard/sfw/blob/main/gap/R).

- [x] The `scatter_plot()` utility function file end with a `_utils` suffix:
...
- [x] Include tests for new modules and functions using the `with_test = TRUE` argument
...
- [x] To include other files (like images), add the image file to `inst/app/www/`, then add the `www/` to the path (see example UI code below)
...
- [x] In `R/app_ui.R`, the `app_ui()` function contains the UI layout functions (`fluidPage()`, `sidebarLayout()`, etc.), and a call to `golem_add_external_resources()`:
...
3. [`dev/`](https://github.com/mjfrigaard/sfw/tree/main/_apps/gap/dev) contains `golem`'s 'guided tour' scripts (`01_start.R`, `02_dev.R`, `03_deploy.R`) and [`run_dev.R`](https://github.com/ThinkR-open/golem/blob/HEAD/R/run_dev.R) (covered above)

4. [`R/`](https://github.com/mjfrigaard/sfw/tree/main/_apps/gap/R): the primary app files for the UI and server are stored in the `R/` folder (`R/app_ui.R`, `R/app_server.R`, `R/run_app.R`), as well as the configuration function (`R/app_config.R`):
    
5. [`inst/`](https://github.com/mjfrigaard/sfw/tree/main/_apps/gap/inst): the `inst/` folder holds the `golem-config.yml` and location of any external app files.
