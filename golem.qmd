...
The golem version and description used in this example is below:

```{r}
#| echo: false
#| eval: true
knitr::kable(
  pkg_info(pkgs = c("golem"))
)
```

## Getting started

`golem` app development is tightly coupled with RStudio ![](img/rstudio.png){height=25} so this framework doesn't include examples in Positron ![](img/positron.png){height=25}.
...
If creating a `golem` app from RStudio's ![](img/rstudio.png){height=25} **New Project Wizard**, the following defaults are available:
...
After loading, documenting, and installing `gap`, we can view the app with `run_app()`.
...
![After loading, documenting, and installing `gap`, launch the app with `run_app()`](img/golem_updated_app.png){width='100%' align='center'}
...
Below are descriptions and versions of the `golem` development dependencies used in this example: 

```{r}
#| echo: false
#| eval: true
knitr::kable(
  pkg_info(pkgs = c("usethis", "pkgload", "dockerfiler", "devtools", "roxygen2", "attachment", "rstudioapi", "here", "fs", "desc", "pkgbuild", "processx", "rsconnect", "testthat", "rstudioapi"))
)
```
