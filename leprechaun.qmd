...
`leprechaun` stores external code files the `inst/` folder using a combination of `use_*` and `build()` functions to add functionality to your app.  

We'll cover how this works with the [`packer`](https://packer.john-coene.com/#/) JavaScript example from the [package website](https://leprechaun.opifex.org/#/examples/basic#javascript). 
...
-   [`packer::scaffold_leprechaun()`](https://packer.john-coene.com/#/references/scaffold_leprechaun) initializes the `npm` package manager for JavaScript, installs `webpack`, and adds the necessary JavaScript files and folders:
    ...
    #| code-fold: true
    #| eval: false
    #| code-summary: 'show/hide packer::scaffold_leprechaun() files'
    ├── node_modules/
    │   ├── @discoveryjs
    │   ├── @jridgewell
    │   ├── @types
