...
[`leprechaun`](https://leprechaun.opifex.org/#/) apps are built much like R packages (with `devtools` and `usethis`), but they are designed with the intention of being a 'leaner and smaller' version of `golem`:[^view-lap]
...
[^view-lap]: The coed used to build the `leprechaun` app is [here](https://github.com/mjfrigaard/sfw/tree/main/_apps/lap)
...
The [`leprechaun::scaffold()`](https://leprechaun.opifex.org/#/reference/scaffold) defaults to a `navbarPage()`, but I've switched to a `fluidPage()` for this example. 
...
- [x] The `R/module_var_input.R` file: `plot_dispay` collects the data from `var_input` and creates the plot with the custom `scatter_plot()` function:
...
- [x] The `R/module_plot_display.R` file 
...
- [x] The `R/ui.R` file: the UI includes the `bs_theme()` function (with version set to `5`).
...
- [x] The `R/server.R` file: The server also has the `make_send_message()` function in it by default (more on that below).
...
- [x] `movies.RData` was added to `inst/extdata` and loaded into the package with `usethis::use_data_raw()`
...
- [x] `usethis::use_r()` created `R/utils_plot_display.R` to hold the `scatter_plot()` function  
...
I'll demonstrate how `make_send_message()` function works by walking through the JavaScript example from the [package website](https://leprechaun.opifex.org/#/examples/basic#javascript). 

- [x]  First we have to build the scaffolding for packer by running [`packer::scaffold_leprechaun()`](https://packer.john-coene.com/#/references/scaffold_leprechaun)
...
- [x] Now that the scaffolding is in place, run `leprechaun::use_packer()`:
...
- [x] The final step is to build or 'bundle' the JavaScript files with `leprechaun::build()`
...
- [x] We'll change the `color:` from `red` to green (`#38B44A`) using `$accent: #38B44A;`
...
- [x] Then run [`leprechaun::build()`](https://leprechaun.opifex.org/#/reference/build)
...
-   [x] Adding modules: `leprechaun`'s `add_module()` function doesn't have the consistent naming or prefixes found in `golem::add_module()`, but still reduces a lot of typing if you are creating these files manually. 
